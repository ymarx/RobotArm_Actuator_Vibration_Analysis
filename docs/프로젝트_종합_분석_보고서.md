...
**수집 데이터**:
- **원본 CSV 파일**: 61개 (100W: 30개, 200W: 31개)
- **분석에 사용된 파일**: 57개 (품질 기준 미달 4개 제외)
  - **Normal: 11개 파일**
    - 100W: S00(CCW,CW), S01(CCW,CW), S05(CCW,CW), S06(CCW,CW) = 8개
    - 200W: S02(CW), S03(CCW,CW) = 3개
  - **Abnormal: 46개 파일**
...
**파일명 규칙**:
- 원본: `100W_Sample00 ccw4_2025-11-07 03-43-20.csv`
- 처리 후: `100W_S00_CCW_R4` (Sample→S, 타임스탬프 제거)
...
**레이블링 전략**:
- **파일 레벨 레이블**: 전문가 판정 기준 (Normal=1, Abnormal=0)
- **윈도우 레벨 상속**: 같은 파일의 모든 윈도우가 동일 레이블 보유
- **결과**:
  - 사용 파일: 57개 (Normal 11개 + Abnormal 46개)
  - 윈도우 슬라이딩 적용 (8초, 50% 오버랩)
  - 최종 윈도우: 898개 (Normal: 392개, Abnormal: 506개)
...
**최종 분포** (Phase 3 기준, 898개 윈도우):

| 데이터셋 | 윈도우 수 | Normal (1) | Abnormal (0) | 비율 (N:A) |
|---------|----------|-----------|--------------|-----------|
| **Train** | 695 | 374 | 321 | 1.17:1 |
| **Validation** | 92 | 4 | 88 | 0.05:1 |
| **Test** | 111 | 14 | 97 | 0.14:1 |

**Train Normal 374개 구성**:
- **시간분할 4개 파일** (100W_S00 CCW/CW, 200W_S03 CCW/CW): 80개 윈도우
- **파일분할 6개 파일**: 294개 윈도우
  - 100W_S01_CCW_R4: 25개
  - 100W_S01_CW_R4: 21개
  - 100W_S05_CCW_R4: 18개
...
**클래스 균형 방법**:
- **Oversampling 미사용**: 모든 윈도우는 원본 데이터에서 생성
- **XGBoost scale_pos_weight**: 파라미터로 클래스 가중치 조정 (0.879)
- Normal 10개 파일, Abnormal 56개 파일 → 자연스러운 클래스 불균형
