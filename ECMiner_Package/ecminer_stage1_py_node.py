...
def process_file(
    ...
    # 파일 경로 처리 (상대 경로인 경우 스크립트 디렉토리 기준으로 해석)
    file_path = Path(file_path_str)
    if not file_path.is_absolute():
        # 상대 경로는 SCRIPT_DIR 기준 (ECMiner_Package 폴더 내부)
        # 만약 data/ 폴더가 부모에 있다면 자동으로 찾음
        candidate_paths = [
            SCRIPT_DIR / file_path,  # 같은 폴더 (ECMiner_Package/data/)
            SCRIPT_DIR.parent / file_path,  # 부모 폴더 (프로젝트루트/data/)
        ]
        ...
        for candidate in candidate_paths:
            ...
            # 둘 다 없으면 같은 폴더 기준으로 시도 (오류는 나중에 발생)
...
# ============================================================================
# 입력 데이터 로드 (ECMiner에서 자동으로 제공되는 부분)
# ============================================================================
# ECMiner에서 실행 시 자동으로 로드됨:
# ecmData = pd.read_csv('%temporary_input_csv_file%')

# 로컬 테스트용 (입력 CSV 찾기)
if 'ecmData' not in globals():
    # 여러 위치에서 입력 CSV 찾기
    input_csv_candidates = [
        SCRIPT_DIR / 'ecminer_input_example.csv',  # 1순위: 같은 폴더의 샘플 파일
        SCRIPT_DIR.parent / 'ecminer_input_full.csv',  # 2순위: 부모 폴더의 전체 파일
        SCRIPT_DIR / 'ecminer_input_full.csv',  # 3순위: 같은 폴더의 전체 파일
    ]
    ...
    if input_csv is None:
        error_msg = (
            f"\n{'=' * 60}\n"
            f"오류: 입력 CSV 파일을 찾을 수 없습니다.\n"
            f"{'=' * 60}\n\n"
            f"다음 위치 중 하나에 입력 CSV 파일을 배치하세요:\n\n"
            f"1. {SCRIPT_DIR / 'ecminer_input_example.csv'}\n"
            f"   (권장) ECMiner_Package 폴더 내에 배치\n\n"
            f"2. {SCRIPT_DIR.parent / 'ecminer_input_full.csv'}\n"
            f"   부모 디렉토리에 배치\n\n"
            f"입력 CSV 형식:\n"
            f"  file_path,file_id\n"
            f"  data/100W/Sample00_CW.csv,100W_Sample00_CW_20251107_034124\n"
            f"  data/200W/Sample03_CCW.csv,200W_Sample03_CCW_20251107_112308\n"
            f"  ...\n\n"
            f"자세한 내용은 README_KR.md를 참조하세요.\n"
            f"{'=' * 60}\n"
        )
        raise FileNotFoundError(error_msg)

    ecmData = pd.read_csv(input_csv)
    print(f"입력 CSV 로드 완료: {input_csv.name} ({len(ecmData)}개 파일)")
...
# 레이블 파일 로드
label_file = SCRIPT_DIR / 'ecminer_labels.csv'
if not label_file.exists():
    raise FileNotFoundError(f"레이블 파일을 찾을 수 없습니다: {label_file}")

labels_df = load_label_file(label_file)
print(f"레이블 파일 로드 완료: {len(labels_df)}개 레이블")

# 설정 파일 로드
config_file = SCRIPT_DIR / 'ecminer_config.yaml'
...
for idx, row in ecmData.iterrows():
    try:
        ...
        if len(label_row) == 0:
            raise ValueError(f"레이블 없음: {product} Sample{sample:02d}")
        ...
        # 파일 처리 (label_binary, label_raw, label_weight 전달)
...
if 'ecmData' in globals():
    test_output_path = SCRIPT_DIR.parent / 'ecminer_output_labeled.csv'
    ecmData.to_csv(test_output_path, index=False)
    print(f"\n테스트 출력 저장: {test_output_path}")
# ============================================================================
